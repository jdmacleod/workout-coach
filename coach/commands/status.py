...
from coach.notes.parser import parse_front_matter
...
def _run_status(*, week: str | None = None) -> None:
    ...
    if week:
        ...
        current_monday = jan4 - datetime.timedelta(days=jan4.weekday()) + datetime.timedelta(weeks=iso_week - 1)
        display_week = week
    else:
    ...
    if not plan_file.exists():
        console.print(
            f"No plan for current week. Run [bold]coach plan[/bold] to generate one."
        )
        raise typer.Exit(0)
