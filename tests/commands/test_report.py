...
def test_report_summary_json():
    ...
    with patch("coach.commands.report.console") as mock_console:
        ...
        _run_report(
            subcommand="summary", weeks=4, format="json", type_filter=None, workouts=workouts
        )
...
def test_report_type_filter():
    ...
    # Should not raise
    _run_report(
        subcommand="summary", weeks=8, format="table", type_filter="strength", workouts=workouts
    )
